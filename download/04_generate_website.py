...
# Function to find the matching thumbnail for an HTML file
def find_thumbnail(html_file):
    base_name = os.path.splitext(os.path.basename(html_file))[0]
    for image_file in image_files:
        if base_name in os.path.basename(image_file):
            return os.path.relpath(image_file, output_dir)
    return None
...
for html_file in html_files:
    ...
    thumbnail_path = find_thumbnail(html_file)
    ...
    html_content += '<li class="list-group-item d-flex align-items-center">'
    if thumbnail_path:
        html_content += f'<img src="{thumbnail_path}" alt="Thumbnail" class="mr-3" style="width: 100px; height: auto;">'
    html_content += f'<a href="{relative_path}">{display_name}</a></li>'
